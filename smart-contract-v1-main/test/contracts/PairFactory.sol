...
contract PairFactory is Ownable{
    ...
    function createPair(
        ...
        IPool(pair).initializeVesting(
          _vestingInit  

        );

        uint256 totalToken = _feesCount(_rateSettings[0] , _rateSettings[1] , _capSettings[1] , _liquidityPercent , _feeSettings[0]);

        // uint totalToken = ((_rateSettings[0] * _capSettings[1] / 10**18)).add(((_capSettings[1] * _rateSettings[1] / 10**18) * _liquidityPercent) / 100);
